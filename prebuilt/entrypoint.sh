...
if [ "${WITH_REACT:-1}" = "1" ] && [ -f /melis-conf/enable-react.sh ]; then
  bash /melis-conf/enable-react.sh "$APP_DIR" \
    || echo "[melis-docker] WARNING: React enablement failed — continuing with the legacy back-office only. See the errors above (missing GITHUB_TOKEN?); restart the container to retry."
fi
...
echo "[melis-docker] ============================================================"
echo "[melis-docker]  Melis (pre-built) is ready. Open http://localhost:${HOST_PORT:-8080}"
...
if [ "${WITH_REACT:-1}" = "1" ]; then
  echo "[melis-docker]  React back-office (once installed):"
  echo "[melis-docker]    http://localhost:${HOST_PORT:-8080}/melis-react"
fi
echo "[melis-docker] ============================================================"
