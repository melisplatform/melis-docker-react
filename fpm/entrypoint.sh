...
until DB_HOST="$DB_HOST" DB_USER="$DB_USER" DB_PASS="$DB_PASS" \
  ...
  tries=$((tries + 1))
  if [ "$tries" -ge 60 ]; then
    echo "[melis-docker-react] WARNING: MySQL still unreachable after ~3 min — starting PHP-FPM anyway; the web installer will retry."
    break
  fi
  sleep 3
done
...
echo "[melis-docker-react]  Melis (PHP-FPM) is ready. Open http://localhost:${HOST_PORT:-8080}"
